...
public class ShoppingService {
    ...
    public List<ShoppingItemResponse> getAllItemsInGroup(UUID groupId) {
        ...
        List<ShoppingItem> items = itemRepository.findByShoppingList_ListId(latestList.getListId());
    ...
    // [추가] 프론트엔드 요청용: 그룹에 아이템 바로 추가
    // (리스트가 없으면 자동으로 생성해주고 추가함 -> 트랜잭션으로 안전하게 처리)
    @Transactional
